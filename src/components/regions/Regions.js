...
const Regiones = () => {
  ...
  const { countries, loaded } = useSelector((country) => country.countries, shallowEqual);
  ...
  useEffect(() => {
    ...
    if (loaded) {
      ...
      const regionsData = countryObj[0].regions.map(({
        id, name, today_confirmed, today_new_confirmed, today_new_deaths, date,
      }) => ({
        date,
        id,
        name,
        todayConfirmed: today_confirmed || 0,
    ...
    for (
      ...
      result.push(
        <Region
          ...
          todayConfirmed={regions[i].todayConfirmed}
          todayNewConfirmed={regions[i].todayNewConfirmed}
        />,
      );
    }
    return result;
  ...
  return (
    <div>
      {' '}
      <Header total={`${total} Cases Today`} title={id.toUpperCase()} carteName={id} />
